...
SAVE_FILE_NAME = "YOUR_CSV_FILE_NAME" # Change this to your desired file name

# Load to audio file
pygame.mixer.init()
BREAK_SOUND = pygame.mixer.Sound("Ding.mp3")
...
def floating_window(**kwargs):
    ...
    if open_floating_window == "True" or open_floating_window == True:
        ...
        window.withdraw()
        checked_state.set(0)
    with open("floating_window_checker.txt", "w") as f:
...
def save_data():
    ...
    if crono_mode_activate:
        ...
        # [f"\n{dt.datetime.now().strftime("%m/%d/%Y")}" f"\n{hours:02d}:{minute:02d}:{second:02d} ", saved_note])
            ...
            # pd.DataFrame(saved_data).to_csv(save_file_name, index=False)
...
ico = Image.open('tomato_window.png')
...
class DraggableWindow(Toplevel):
    def __init__(self):
        ...
        self.image = ImageTk.PhotoImage(Image.open("tomato.gif"))
...
window.withdraw()
with open("floating_window_checker.txt", "r") as file:
